const { waitForAnyVisible, waitForSalesNavReady, waitForLeadCountStable } = require("./dom");
...
/**
 * Get total number of result pages from the pagination UI.
 */
...
const clickNextPage = async (page, { timeoutMs = 15000, expectedNext = null } = {}) => {
  ...
  // Wait for URL or page number to change
  try {
  ...
  // Wait for lead list content to change (proves new data loaded)
  if (beforeLeadKey) {
    try {
      const start = Date.now();
      for (;;) {
  ...
  // Wait for full page readiness + lead cards to load
  await waitForSalesNavReady(page, timeoutMs);
  await waitForLeadCountStable(page, { timeoutMs: 5000, stableMs: 600, minCount: 1 }).catch(() => null);
...
const clickNextPageWithRetry = async (
  page,
  { timeoutMs = 15000, expectedNext = null, maxRetries = 3 } = {}
) => {
  let lastResult = null;
